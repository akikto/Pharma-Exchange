    ...
    <div>
      ...
      <div className="sticky top-14 z-30 bg-surface-base px-4 py-3 border-b border-border-subtle">
        <form onSubmit={handleSearch} className="flex gap-2">
          ...
          <Button type="button" variant="secondary" size="icon" onClick={() => setShowFilters(!showFilters)}>
        ...
        {showFilters && (
          <div className="mt-3 p-3 rounded-[var(--radius-md)] border border-border-subtle space-y-3">
            <Input placeholder="City" defaultValue={params.get('city') || ''} onBlur={(e) => { const p = new URLSearchParams(params); if (e.target.value) p.set('city', e.target.value); else p.delete('city'); setParams(p); }} />
            <Input placeholder="Min discount %" type="number" defaultValue={params.get('minDiscount') || ''} onBlur={(e) => { const p = new URLSearchParams(params); if (e.target.value) p.set('minDiscount', e.target.value); else p.delete('minDiscount'); setParams(p); }} />
            <select className="w-full h-10 rounded-[var(--radius-md)] border border-border-subtle px-3 text-sm" value={params.get('sortBy') || 'createdAt'} onChange={(e) => { const p = new URLSearchParams(params); p.set('sortBy', e.target.value); setParams(p); }}>
