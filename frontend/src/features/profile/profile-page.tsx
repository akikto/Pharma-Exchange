      ...
      <div className="p-4 space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {user?.pharmacy && <p className="text-xs text-primary mt-0.5">{user.pharmacy.name}</p>}
          </div>
        </div>

        {isVerified && (
          <div className="flex gap-2 p-1 bg-surface-sunken rounded-[var(--radius-md)]">
            <button className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-[var(--radius-sm)] text-sm ${mode === 'buyer' ? 'bg-surface-base shadow-sm font-medium' : ''}`} onClick={() => setMode('buyer')}>
              <ShoppingBag className="h-4 w-4" /> Buying
            </button>
            <button className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-[var(--radius-sm)] text-sm ${mode === 'seller' ? 'bg-surface-base shadow-sm font-medium' : ''}`} onClick={() => setMode('seller')}>
