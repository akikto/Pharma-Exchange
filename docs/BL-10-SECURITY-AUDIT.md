...
- `minimatch` ^10.2.3
- `smol-toml` ^1.6.1
- `uuid` ^11.1.1 (where npm hoisting allows)
...
```bash
npm run audit:ci   # npm audit --audit-level=high --omit=dev
```

CI runs `npm run audit:ci` after `npm ci`. Remaining findings are documented below; the gate prevents **new** high-severity production-runtime issues from merging unnoticed.
...
|---|---|---|---|
| GHSA-qwww-vcr4-c8h2 | `react-router` 7.18.2 | High (false positive) | **Patched in 7.18.2**; only affects unstable RSC APIs (not used). npm advisory DB range not yet updated. |
| GHSA-w5hq-g745-h8pq | `uuid` via `firebase-admin` → `@google-cloud/storage` | Moderate | Nested `teeny-request` pins `uuid@9`; override does not propagate. Runtime uses Firebase server SDK only; no direct `uuid` buffer API from app code. Awaiting upstream `google-gax` / `firebase-admin` release. |
